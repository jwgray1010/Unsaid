...
class KeyboardDataSyncWidget extends StatefulWidget {
  final Widget child;
  final Function(KeyboardDataService.KeyboardAnalyticsData)? onDataReceived;
...
class _KeyboardDataSyncWidgetState extends State<KeyboardDataSyncWidget> 
  ...
  Future<void> _performDataSync() async {
    ...
    try {
      ...
      final keyboardData = await _keyboardDataService.retrievePendingKeyboardData();
      
      if (keyboardData != null && keyboardData.hasData) {
        debugPrint('📥 KeyboardDataSyncWidget: Retrieved ${keyboardData.totalItems} items');
        ...
        if (cleared) {
          debugPrint('✅ KeyboardDataSyncWidget: Data sync completed successfully');
        } else {
          debugPrint('⚠️ KeyboardDataSyncWidget: Warning - data not cleared after sync');
        }
      } else {
