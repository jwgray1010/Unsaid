...
class ModernPersonalityResultsScreen extends StatefulWidget {
  ...
  @override
  State<ModernPersonalityResultsScreen> createState() =>
      _ModernPersonalityResultsScreenState();
}

class _ModernPersonalityResultsScreenState
    extends State<ModernPersonalityResultsScreen>
  ...
  Future<void> _savePersonalityResults() async {
    try {
      ...
      // Also push to iOS for keyboard extension
      const channel = MethodChannel('com.example.unsaid/personality');
      await channel.invokeMethod('storeModernPersonality', {
        'anxiety_score': widget.scores.anxiety,
        'avoidance_score': widget.scores.avoidance,
        'attachment_quadrant': widget.scores.quadrant,
        'primary_profile': widget.config.primaryProfile,
      ...
      print('Modern personality test results saved successfully');
    } catch (e) {
  ...
  Widget _buildScoreCard(String title, String value, String subtitle, Color color, IconData icon) {
                ...
                RadarChartData(
                  dataSets: [
                    RadarDataSet(
                      ...
                      fillColor: _getQuadrantColor(widget.scores.quadrant).withOpacity(0.2),
                  ...
                  getTitle: (index, angle) {
                    switch (index) {
                      case 0:
                        return RadarChartTitle(text: 'Anxiety\n${widget.scores.anxiety}%');
                      case 1:
                        return RadarChartTitle(text: 'Avoidance\n${widget.scores.avoidance}%');
                      case 2:
                        return RadarChartTitle(text: 'Reliability\n${(widget.scores.reliabilityAlpha * 100).round()}%');
                      default:
        ...
        child: Column(
          ...
          children: [
            ...
            _buildQualityRow(
              'Reliability (α)',
              '${(widget.scores.reliabilityAlpha * 100).round()}%',
              widget.scores.reliabilityAlpha > 0.7 ? Icons.psychology : Icons.warning,
              widget.scores.reliabilityAlpha > 0.7 ? Colors.green : 
              widget.scores.reliabilityAlpha > 0.6 ? Colors.orange : Colors.red,
            ),
            ...
            _buildQualityRow(
              ...
              widget.scores.confidenceLabel == 'High' ? Icons.star : 
              widget.scores.confidenceLabel == 'Moderate' ? Icons.star_half : Icons.star_border,
              widget.scores.confidenceLabel == 'High' ? Colors.green : 
              widget.scores.confidenceLabel == 'Moderate' ? Colors.orange : Colors.red,
            ),
          ],
        ),
      ),
  ...
  Widget _buildQualityRow(String label, String value, IconData icon, Color color) {
    return Padding(
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                      child: _buildScoreCard(
                        'Goal Profile',
                        widget.config.primaryProfile.replaceAll('_', ' ').toUpperCase(),
                        'Communication focus',
