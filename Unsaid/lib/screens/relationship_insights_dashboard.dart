  ...
  List<Map<String, dynamic>> _getPersonalizedInsights() {
      ...
      insights.addAll([
        {
          ...
          'description':
              'You tend to keep messages short and withdraw. Try adding one validating line before ending the conversation: "I care about this, I just need a moment to process."'
        },
        {
          'icon': Icons.chat_bubble_outline,
          'title': 'Connection Strategy',
          'description':
              'Your independence is valuable, but small gestures of emotional availability can strengthen your bond without feeling overwhelming.'
        },
      ]);
      ...
      insights.addAll([
        {
          ...
          'title': 'When Worried',
          'description':
              'You may send longer messages seeking reassurance. Try stating your need clearly: "I\'m feeling uncertain and would love to hear that we\'re okay."'
        },
        {
          'icon': Icons.self_improvement,
          'title': 'Self-Soothing',
          'description':
              'Before sending that third follow-up, take a breath. Your partner\'s delayed response usually isn\'t about you.'
        },
      ]);
      ...
      insights.addAll([
        {
          ...
          'title': 'Mixed Signals',
          'description':
              'You might alternate between seeking closeness and creating distance. Acknowledging this pattern can help: "I\'m feeling conflicted right now."'
        },
        {
          'icon': Icons.healing,
          'title': 'Safety First',
          'description':
              'Grounding techniques before difficult conversations can help you stay present: "Let me take a moment to center myself."'
        },
      ]);
    } else {
      // secure
      insights.addAll([
        {
          ...
          'title': 'Your Strength',
          'description':
              'Your natural ability to communicate needs and stay emotionally regulated is a gift to your relationship.'
        },
        {
          'icon': Icons.support,
          'title': 'Supporting Growth',
          'description':
              'You can help model healthy communication patterns while being patient with your partner\'s attachment style.'
        },
      ]);
    }
    ...
    if (communicationStyle.toLowerCase().contains('direct')) {
      insights.add({
        ...
        'description':
            'Your clarity is helpful, but softening with empathy can prevent defensiveness: "I notice..." instead of "You always..."'
      });
      ...
      insights.add({
        ...
        'description':
            'Your considerate style creates safety. Don\'t forget to also express your own needs clearly.'
      });
    }
...
class ConflictHeatmap extends StatelessWidget {
                  ...
                  Expanded(
                    child: GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return GestureDetector(
                          onTap: () =>
                              _showHeatmapDetail(context, day, hour, value),
                          child: Container(
                            decoration: BoxDecoration(
  ...
  void _showHeatmapDetail(
      BuildContext context, int day, int hour, double value) {
    final dayLabels = [
      'Monday',
      'Tuesday',
      'Wednesday',
  ...
  String _getBestWorstTimes(List<List<double>> matrix) {
    ...
    final worstPeriod =
        worstHour < 12 ? 'morning' : (worstHour < 18 ? 'afternoon' : 'evening');
    final bestPeriod =
        bestHour < 12 ? 'morning' : (bestHour < 18 ? 'afternoon' : 'evening');
...
class RuptureRepairTracker extends StatelessWidget {
        ...
        child: Column(
          ...
          children: [
                        ...
                        children: [
                          Text(
                            ...
                            style: Theme.of(context)
                                ...
                                .headlineMedium
                                ?.copyWith(
                                  color: _getRepairRateColor(
                                      repairData['repairRate']),
                                  fontWeight: FontWeight.bold,
                                ),
                          ),
                          const SizedBox(width: 8),
                ...
                children: [
                  ...
                  LinearProgressIndicator(
                    value: repairData['weeklyRuptures'] > 0
                        ? repairData['weeklyRepairs'] /
                            repairData['weeklyRuptures']
                        : 0,
            ...
            if (repairData['recentRuptures'].isNotEmpty) ...[
              ...
              const SizedBox(height: 8),
              ...repairData['recentRuptures']
                  .take(3)
                  .map<Widget>((rupture) => _buildRuptureItem(context, rupture))
                  .toList(),
            ],
          ],
        ),
  ...
  Map<String, dynamic> _calculateRepairMetrics() {
    ...
    for (final rupture in ruptures) {
      final ruptureTime =
          DateTime.tryParse(rupture['timestamp'] as String? ?? '');
      ...
      final hasRepair = recentAnalyses.any((analysis) {
        final analysisTime =
            DateTime.tryParse(analysis['timestamp'] as String? ?? '');
...
class _MicroHabitsExperimentState extends State<MicroHabitsExperiment> {
  ...
  Future<void> _loadCurrentExperiment() async {
    ...
    final savedExperiment =
        await storage.getString('current_micro_habit_experiment');
  ...
  Future<void> _startExperiment() async {
    ...
    await storage.setString(
        'current_micro_habit_experiment', selectedExperiment!);
    await storage.setString('micro_habit_day', '1');
    await storage.setString('micro_habit_completed_uses', '0');
    await storage.setString(
        'micro_habit_start_date', DateTime.now().toIso8601String());
  ...
  Future<void> _logExperimentUse(bool successful) async {
    ...
    if (successful) {
      completedUses++;
      await storage.setString(
          'micro_habit_completed_uses', completedUses.toString());
  ...
  final Map<String, Map<String, dynamic>> experiments = {
    'pause_before_react': {
      ...
      'science':
          'Activates prefrontal cortex, reduces reactive responses by 40%',
    },
    'appreciation_sandwich': {
      'title': 'Appreciation Sandwich',
      'description':
          'Start difficult conversations with something you appreciate',
      'icon': Icons.favorite_outline,
  ...
  Widget _buildExperimentOption(
      BuildContext context, String key, Map<String, dynamic> experiment) {
    final colorScheme = Theme.of(context).colorScheme;
...
class _RelationshipInsightsDashboardState
    extends State<RelationshipInsightsDashboard> with TickerProviderStateMixin {
  // Controllers
  TabController? _tabController;
  late AnimationController _animationController;

  // Services
  final RelationshipInsightsService _insightsService =
  ...
  final List<BarChartGroupData> _goalsProgressData = [];
  ...
  Future<void> _loadInsightsData() async {
    ...
    try {
      ...
      final realKeyboardData =
          await _keyboardManager.getComprehensiveRealData();
      final relationshipInsights =
          await _insightsService.generateRelationshipInsights();
      await _analyticsService.getRelationshipAnalytics();

      if (mounted) {
        setState(() {
          // Check if user has real data
          if (realKeyboardData['real_data'] == true &&
              (realKeyboardData['total_interactions'] ?? 0) > 0) {
      ...
      if (mounted) {
        setState(() {
          _error =
              null; // Don't show error to new users - show encouragement instead
          _isLoading = false;
  ...
  Map<String, dynamic> _generateFallbackInsights() {
    return {
      ...
      'newUserMessages': {
        ...
        'subtitle':
            'Your personalized relationship dashboard will develop as you use Unsaid',
        'encouragement':
            'Start conversations with the Unsaid keyboard to unlock insights about your communication patterns',
        'nextSteps': [
  ...
  void _generateCommunicationBalanceData() {
    ...
    final partnerService =
        Provider.of<PartnerDataService>(context, listen: false);
    final analysisHistory = _keyboardManager.analysisHistory;
    ...
    if (total > 0) {
      _communicationBalanceData = [
        ...
        PieChartSectionData(
          ...
          title:
              '${partnerService.partnerName ?? 'Partner'}\n${(partnerMessages / total * 100).round()}%',
          titleStyle: RelationshipInsightsUtils.chartTitleStyle,
          radius: 80,
        ),
      ];
  ...
  void _generateGoalsProgressData() async {
    ...
    try {
      ...
      if (analysisHistory.isNotEmpty) {
        ...
        double qualityTimeProgress =
            _calculateQualityTimeProgress(recentAnalyses);
  ...
  double _calculateListeningProgress(List<Map<String, dynamic>> analyses) {
    ...
    for (final analysis in analyses) {
      ...
      if (text.contains('understand') ||
          text.contains('hear you') ||
          text.contains('what you mean') ||
  ...
  double _calculateQualityTimeProgress(List<Map<String, dynamic>> analyses) {
    ...
    for (final analysis in analyses) {
      ...
      if (text.contains('together') ||
          text.contains('time with') ||
          text.contains('date') ||
          text.contains('spend time') ||
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer2<NewUserExperienceService, PartnerDataService>(
      builder: (context, newUserService, partnerService, child) {
        return Scaffold(
          appBar: AppBar(
            title: Text(_relationshipType == 'couples'
                ? 'Couple Insights'
                : 'Co-Parent Insights'),
                ...
                IconButton(
                  ...
                  onPressed: () {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content:
                            Text('Connected to ${partnerService.partnerName}'),
                      ),
                    );
                  },
                ),
          ...
          body: _isLoading
              ...
              : _error != null
                  ...
                  : TabBarView(
                      ...
                      children: [
                        _buildOverviewTab(
                            theme, partnerService, newUserService),
                        _buildAnalyticsTab(
                            theme, partnerService, newUserService),
          ...
          floatingActionButton: FloatingActionButton.extended(
            ...
            label: Text(_relationshipType == 'couples'
                ? 'Couple Tips'
                : 'Co-Parent Tips'),
  ...
  Widget _buildOverviewTab(ThemeData theme, PartnerDataService partnerService,
      NewUserExperienceService newUserService) {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          if (!partnerService.hasPartner) const SizedBox(height: 24),
  ...
  Widget _buildAnalyticsTab(ThemeData theme, PartnerDataService partnerService,
      NewUserExperienceService newUserService) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
  ...
  Widget _buildGoalsTab(ThemeData theme, PartnerDataService partnerService,
      NewUserExperienceService newUserService) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
  ...
  Widget _buildRelationshipHealthSummary(
      ThemeData theme, PartnerDataService partnerService) {
  ...
  List<Widget> _generateRecentActivityItems(
      ThemeData theme, PartnerDataService partnerService) {
    ...
    if (combinedHistory.isEmpty) {
      ...
      // Communication trend
      final positiveCount = recentAnalyses
          .where((a) => ['happy', 'loving', 'supportive', 'grateful']
              .contains(a['emotional_tone']))
          .length;
  ...
  Widget _buildActivityItem(ThemeData theme, String title, String subtitle,
              ...
              builder: (context, snapshot) {
                    ...
                    child: Column(
                      children: [
                        ...
                        ElevatedButton.icon(
                          onPressed: () => _showAddChildDialog(theme),
                          icon: const Icon(Icons.add),
                          label: Text(_relationshipType == 'co-parents'
                              ? 'Add Child'
                              : 'Add Name'),
                        ),
                      ],
                ...
                return Wrap(
                  ...
                  children: childrenNames
                      .map((name) => _buildChildNameChip(theme, name))
                      .toList(),
                );
              },
            ),
  ...
  void _showAddChildDialog(ThemeData theme) {
    ...
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(_relationshipType == 'co-parents'
            ? 'Add Child\'s Name'
            : 'Add Family Name'),
        content: Form(
          key: formKey,
            ...
            children: [
              TextFormField(
                ...
                decoration: InputDecoration(
                  labelText: 'Name',
                  hintText: _relationshipType == 'co-parents'
                      ? 'Enter child\'s name'
  ...
  double _calculateRelationshipHealthScore(PartnerDataService partnerService) {
    ...
    final baseScore =
        validAnalyses > 0 ? (totalScore / validAnalyses).clamp(0.0, 1.0) : 0.80;
  ...
  String _getCommunicationScore() {
    ...
    final positiveMessages = recentMessages
        .where((msg) => [
              'happy',
              'supportive',
              'loving',
              ...
              'grateful',
              'excited'
            ].contains(msg['emotional_tone']?.toString().toLowerCase()))
        .length;
  ...
  Widget _buildRelationshipProgressChart(
      ThemeData theme, PartnerDataService partnerService) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
  ...
  Widget _buildCommunicationBalanceChart(
      ThemeData theme, PartnerDataService partnerService) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
  ...
  Widget _buildSecureCommunicationInsights(
      ThemeData theme, PartnerDataService partnerService) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
  ...
  List<Widget> _generateSecureCommunicationInsights(
      ThemeData theme, PartnerDataService partnerService) {
    final combinedHistory = partnerService.hasPartner
        ? partnerService.getCombinedAnalysisHistory()
        : _keyboardManager.analysisHistory;
    ...
    if (combinedHistory.isEmpty) {
      ...
      final emotionalMessages = recentAnalyses
          .where((a) => ['happy', 'grateful', 'calm', 'understanding']
              .contains(a['emotional_tone']))
          .length;
      final emotionalPercentage =
          ((emotionalMessages / recentAnalyses.length) * 100).round();
      ...
      final clarityScore = recentAnalyses.fold<double>(
              0, (sum, a) => sum + (a['confidence'] as double? ?? 0.5)) /
  ...
  Widget _buildCoupleSpecificInsight(
      ThemeData theme, List<Map<String, dynamic>> analyses) {
  ...
  Widget _buildCoParentSpecificInsight(
      ThemeData theme, List<Map<String, dynamic>> analyses) {
  ...
  Widget _buildInsightTile(ThemeData theme, String title, String description,
      IconData icon, Color color) {
    return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            ConstrainedBox(
              ...
              child: const RelationshipCoupleGoals(),
            ),
          ],
        ),
      ),
  ...
  Widget _buildSecureCommunicationTips(
      ThemeData theme, PartnerDataService partnerService) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ..._getPersonalizedSecureCommunicationTips().map(
              (tip) => _buildTipTile(theme, tip),
            ),
            const SizedBox(height: 12),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton.icon(
                ...
                label: Text(
                    'View All ${_relationshipType == 'couples' ? 'Couple' : 'Co-Parent'} Tips'),
                style: ElevatedButton.styleFrom(
                  padding: const EdgeInsets.all(16),
                ),
              ),
  ...
  List<Map<String, dynamic>> _getPersonalizedSecureCommunicationTips() {
    ...
    final emotionalTones =
        recentAnalyses.map((a) => a['emotional_tone']).toSet();

    List<Map<String, dynamic>> personalizedTips = [];

    // Add tips based on emotional patterns
    if (emotionalTones.contains('angry') ||
        emotionalTones.contains('frustrated')) {
      personalizedTips.add({
        'title': 'Emotional Regulation',
        'description':
            'Take deep breaths before responding when feeling intense emotions',
        'icon': Icons.self_improvement,
        'color': Colors.orange,
      });
    }
      ...
      return [
        {
          ...
          'description':
              'Keep discussions centered on your children\'s wellbeing',
        ...
        },
        {
          ...
          'description':
              'Coordinate schedules transparently to avoid conflicts',
        ...
        },
        {
          ...
          'description':
              'Maintain appropriate boundaries in your co-parenting relationship',
          'icon': Icons.shield,
          'color': Colors.orange,
        },
      ];
  ...
  Widget _buildPartnerInviteCard(
      ThemeData theme, PartnerDataService partnerService) {
    return Card(
      child: Container(
            ...
            onPressed: () async {
              if (codeController.text.isNotEmpty &&
                ...
                if (success && mounted) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                        content: Text('Connected to ${nameController.text}!')),
                  );
...
class CoParentingIntelligence extends StatelessWidget {
              ...
              child: Column(
                children: [
                    ...
                    children: [
                      Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 4),
                          Text(
                            _getChildFirstDescription(
                                insights['childFirstScore']),
                            style:
                      ...
                      Container(
                        padding: const EdgeInsets.all(16),
                        decoration: BoxDecoration(
                          color: RelationshipInsightsUtils.getScoreColor(
                                  insights['childFirstScore'])
                              .withOpacity(0.2),
                  ...
                  LinearProgressIndicator(
                    ...
                    backgroundColor: colorScheme.surfaceContainerHighest,
                    valueColor: AlwaysStoppedAnimation<Color>(
                      RelationshipInsightsUtils.getScoreColor(
                          insights['childFirstScore']),
                    ),
                  ),
                ],
  ...
  Map<String, dynamic> _analyzeCoParentingPatterns() {
    ...
    final unityScore =
        totalMessages > 0 ? 1.0 - (conflictMessages / totalMessages) : 1.0;
  ...
  double _calculateChildShieldScore() {
    ...
    for (final analysis in analyses) {
      ...
      final hasProtectiveLanguage =
          protectiveKeywords.any((keyword) => text.contains(keyword));
      ...
      final isExposing =
          exposingKeywords.any((keyword) => text.contains(keyword));
  ...
  double _calculateScheduleHarmony() {
    ...
    for (final analysis in analyses) {
      ...
      if (schedulingKeywords.any((keyword) => text.contains(keyword))) {
        ...
        if (tone != 'alert' &&
            cooperativeKeywords.any((keyword) => text.contains(keyword))) {
  ...
  double _calculateEmotionalSafety() {
    ...
    for (final analysis in analyses) {
      ...
      if (emotionalKeywords.any((keyword) => text.contains(keyword))) {
        ...
        final isIStatement =
            safeEmotionalKeywords.any((keyword) => text.contains(keyword));
