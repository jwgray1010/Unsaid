...
class _CoParentingCoachScreenState extends State<CoParentingCoachScreen>
    with SingleTickerProviderStateMixin {
  final TextEditingController _messageController = TextEditingController();
  final KeyboardManager _keyboardManager = KeyboardManager();
  bool _isAnalyzing = false;
  String _currentTone = 'neutral';
  double _toneConfidence = 0.0;
  List<String> _realTimeSuggestions = [];
  ...
  void initState() {
    ...
    // Listen for text changes to provide real-time analysis
  ...
  Future<void> _performRealTimeAnalysis() async {
    ...
    try {
      final analysis = await _keyboardManager.performComprehensiveAnalysis(
        text,
        attachmentStyle: widget.userAttachmentStyle,
        communicationStyle: widget.userCommunicationStyle,
      );

      if (analysis.containsKey('error')) {
        print('Analysis error: ${analysis['error']}');
        return;
      }

      setState(() {
        _liveAnalysis = analysis;
        _currentTone = analysis['tone_analysis']['dominant_tone'] ?? 'neutral';
        _toneConfidence = analysis['tone_analysis']['clarity_score'] ?? 0.0;
        _realTimeSuggestions =
            (analysis['integrated_suggestions'] as List<dynamic>)
                .map((s) => s['title'].toString())
                .toList();
      });

      // Show coach if there are important suggestions
  ...
  void _hideCoach() {
    if (_coachVisible) {
      _coachController.reverse().then((_) {
        setState(() {
          _coachVisible = false;
        });
      });
