...
// MARK: - Keyboard Layout and UI Types

/// Represents keyboard layout styles
enum KeyboardLayoutStyle: String, CaseIterable, Codable {
    case overlay = "overlay"
...
/// Represents tone indicator positions
enum ToneIndicatorPosition: String, CaseIterable, Codable {
    case topLeft = "topLeft"
    case topRight = "topRight"
    case bottomLeft = "bottomLeft"
...
/// Manager for personality test data shared between main app and keyboard
class PersonalityDataManager {
    private static let userDefaults = UserDefaults(suiteName: "group.com.unsaid.shared")
    private static let personalityTestKey = "personality_test_results"
    
    /// Save personality test results from main app
    static func savePersonalityTestResults(_ results: PersonalityTestResults) {
        let dict = results.toDictionary()
        userDefaults?.set(dict, forKey: personalityTestKey)
        userDefaults?.synchronize()
    }
    
    /// Get personality test results for keyboard use
    static func getPersonalityTestResults() -> PersonalityTestResults? {
        guard let dict = userDefaults?.dictionary(forKey: personalityTestKey) else {
            return nil
        }
        return PersonalityTestResults.fromDictionary(dict)
    }
    
    /// Check if personality test has been completed
    static func hasPersonalityTestResults() -> Bool {
        return getPersonalityTestResults() != nil
    }
    
    /// Clear personality test results (for testing or reset)
    static func clearPersonalityTestResults() {
        userDefaults?.removeObject(forKey: personalityTestKey)
        userDefaults?.synchronize()
    }
}
...
/// Conversation trajectory for predictive analysis
struct ConversationTrajectory: Codable {
    ...
    init(
        ...
        predictedOutcome: String = "Neutral",
        timeframe: PredictionTimeframe = .hours
    ) {
...
struct CrisisPreventionPrediction: Codable {
    let riskLevel: UrgencyLevel
    let triggerPoints: [String]
    ...
    init(
        ...
        timeToIntervention: TimeInterval = 0,
        confidence: Double = 0.5
    ) {
        self.riskLevel = riskLevel
        self.triggerPoints = triggerPoints
        self.preventionStrategies = preventionStrategies
        self.timeToIntervention = timeToIntervention
        self.confidence = confidence
    }
}
